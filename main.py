...
const CIRC_PRESETS = {
  ...
  circadian_strips: [
    ...
    "then each episode strip in time order. Each x-axis runs 0 to 30 seconds.",
    "Y label 'PPG (filtered)', x label 'Seconds within strip'. Build it in one pass",
    "with a single subplots call.",
    "",
    "PANEL TITLES — large and readable, not a caption. Each panel gets a bold title",
    "at fontsize 14, left aligned, naming what the strip is and the recording it came",
    "from:",
    "    Sinus control — 2026-07-22 12:47",
    "    Irregular rhythm episode 1 — 2026-07-22 12:47",
    "Put the raw strip_id in the top-right corner of the panel instead, as a small",
    "grey annotation at fontsize 7, colour '#888888'. It is there for traceability",
    "and must not compete with the title.",
    "",
    "Y-AXIS — scale each panel to its OWN trace. Panels do NOT share a y-scale. A",
    "clean low-amplitude strip must fill its panel rather than sit as a flat line",
    "inside a range set by a noisier strip elsewhere in the figure. For each panel,",
    "from that strip's filtered samples y:",
    "    lim = 1.15 * float(np.percentile(np.abs(y), 99.5))",
    "    lim = max(lim, 1000.0)",
    "    ax.set_ylim(-lim, lim)",
    "The 99.5th percentile stops a handful of motion spikes from setting the scale",
    ...
    "",
    "WHEN NO EPISODE STRIPS EXIST — a sinus control strip with no episode strips",
    "beneath it is the normal, expected result, and the figure must say so rather",
    "than leave the reader wondering whether panels are missing. Draw the sinus panel",
    "as above, then place a single line of text in the figure directly below it,",
    "centred, fontsize 11, colour '#444444':",
    "    No irregular rhythm episode strips in this selection.",
    "Use fig.text below the sinus axes, and leave room for it with the subplots",
    "layout so it is not clipped. Nothing else on that line, no box, no explanation.",
    "",
